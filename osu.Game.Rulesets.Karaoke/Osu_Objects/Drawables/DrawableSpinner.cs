...
using osu.Game.Rulesets.Karaoke.Osu_Objects.Drawables.Pieces;
...
namespace osu.Game.Rulesets.Karaoke.Osu_Objects.Drawables
{
    public class DrawableSpinner : DrawableOsuHitObject
    {
        ...
        protected override void CheckForJudgements(bool userTriggered, double timeOffset)
        {
            ...
            if (!userTriggered && Time.Current >= spinner.EndTime)
            {
                if (Progress >= 1)
                    ...
                    AddJudgement(new KaraokeJudgement { Result = HitResult.Good });
                else if (Progress > .75)
                    AddJudgement(new KaraokeJudgement { Result = HitResult.Meh });
        ...
        {
            disc.Tracking = KaraokeActionInputManager.PressedActions.Any(x => x == KaraokeAction.LeftButton || x == KaraokeAction.RightButton);
